-- Fall back to the playbook owner's display name when no member is
-- explicitly flagged as head coach, so invite landings show a real person
-- instead of "—" in the common case where the owner hasn't set the head
-- coach flag yet.
  ...
  select
    ...
    coalesce(
      ...
      (
        ...
        from public.playbook_members pm
        left join public.profiles pr on pr.id = pm.user_id
        where pm.playbook_id = p.id and pm.role = 'owner'
