...
const WRITE_SECTION = [
  ...
  "- You can perform some write actions — sending announcements, renaming the league, setting the registration link — but EVERY action requires the operator's explicit approval.",
  "- Before proposing a write, gather the facts that matter. For an announcement, first call announcement_audiences so you can tell the operator how many people it will reach.",
  "- To propose an action, call its tool (send_announcement, send_group_announcement, rename_league, set_registration_link). Calling the tool does NOT execute it — it shows the operator an Approve button.",
