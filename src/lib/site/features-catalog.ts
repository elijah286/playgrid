...
export const FEATURES: FeatureEntry[] = [
  ...
  // ── Plays & formations ──────────────────────────────────────────
  {
    ...
    name: "Formations for defense and special teams",
    description:
      "Formations now cover all three sides of the ball, not just offense. The new-play dialog offers a library of real defensive looks for the playbook's game type — Cover 2, Cover 3, Tampa 2, Cover 4, Cover 0, blitz packages, and in tackle the full fronts (4-3 Over, 3-4, Nickel, 46 Bear, 4-4 Stack) — each drawn from the same catalog that powers Coach Cal and the Football Library, so the alignment is genuinely correct rather than a rough sketch. Start a play from one and the coverage's zones come pre-drawn. Coaches can design their own too: creating a formation asks which side it's for (offense, defense, or — in tackle — special teams) and the field adapts, drawing receivers as circles, defenders as red triangles, and the punt or kick unit as blue squares, with the right positions available for each. The Formations tab badges and filters by side, so a defensive look is identifiable at a glance. Every defensive play shows its formation in the header and can be swapped to another: the defenders re-align to the new front and that coverage's zones come with them. Because a defensive front is a personnel change rather than a rearrangement of the same players, swapping asks first if there are defender paths to lose.",
  ...
  },
  {
    ...
    description:
      "Photograph a printed play sheet (Playmaker X and similar exports) or a clear hand-drawn play, and the importer reads it into a real, editable play: it finds each play panel on the sheet, identifies every player's route from the app's own route catalog (family, depth, break direction), places players exactly where the photo shows them (pre-snap jet motion drawn as the dashed zig-zag), and drafts the play in the target playbook's format. Nothing saves automatically — the coach reviews the draft side-by-side with the photo, with a confidence badge on every route and one-tap corrections, then saves. Reads run as resumable background jobs: leave the page mid-read and pick the result up from \"Recent imports.\" Full photos are never stored; the cropped panel of an in-flight import is held privately for up to 24 hours, then deleted. Metered per month; behind the photo_play_import beta flag.",
    category: "Plays & formations",
    status: "beta",
