...
- **Route/token colors** — the renderer auto-colors offensive tokens role-first, then by label. Convention: QB white, C black, RB (B / HB / single back) purple, FB orange, TE green, X red, Z blue, Y green (TE-equivalent), and any other slot label (S / A / H / F-as-WR) yellow. Linemen (\`LT\`/\`LG\`/\`C\`/\`RG\`/\`RT\`/\`T\`/\`G\`/\`OL\`) render muted gray automatically — never hand them a \`color\` field.
- **NO TWO SKILL-POSITION PLAYERS MAY SHARE A DERIVED COLOR.** This is a hard chat-time validator gate. The slot family (\`S\`, \`A\`, \`H\`, and \`F\` when role≠RB) all derive to YELLOW — two slot players in one play is a clash; relabel one to a back (\`B\` purple, \`FB\` orange) or recolor with \`set_player_color\`. \`X + X2\` (both red), \`Z + Z2\` (both blue), \`B + B2\` (both purple) all clash too. When a play needs a 4th or 5th skill player beyond X/Y/Z, prefer adding the back (\`B\` purple) and one slot (\`H\` yellow) before reaching for a second slot. A 5-wide spread should usually carry X (red) + Y (green) + Z (blue) + H (yellow slot) + B (purple) — five distinct hues. Two backs (HB + FB) reads cleanly: \`B\` (purple) + \`FB\` (orange). The validator will reject the fence and force re-emit if you ship a clash.
...
- **Color clash (HARD, save-time):** Two skill-position offensive players sharing a derived token color (two slots both → yellow, two X's both → red, etc.) is now rejected at save-time, not just at chat-time. When you need a 4th or 5th skill player beyond X/Y/Z, use distinct hues: prefer adding @B (purple) and one slot @H (yellow) before reaching for a second slot label. Never ship @H + @S in the same play (both yellow); relabel one to @F-with-role-RB (purple) or @FB (orange), or call \`revise_play\` with \`set_player_color\` on one of them. The save-time gate produces the same error and forces a re-emit — \`create_play\` will not save through the clash.
