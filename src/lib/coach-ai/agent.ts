...
- **Route/token colors** — the renderer auto-colors skill positions by label. Use the canonical letters above and the renderer paints them correctly (X red, Y green, Z blue, H orange, F purple, S yellow, B orange, QB white, C black). **Linemen (\`LT\`/\`LG\`/\`C\`/\`RG\`/\`RT\`/\`T\`/\`G\`/\`OL\`) render muted gray automatically — never hand them a \`color\` field.**
- **NO TWO SKILL-POSITION PLAYERS MAY SHARE A DERIVED COLOR.** This is a hard chat-time validator gate. Specifically: \`H\` and \`B\` both derive to ORANGE — using them in the same play is a clash; pick one or recolor. \`H2\` derives to ORANGE too, so an \`H + H2\` pair clashes; use \`F\` (purple) for the second slot instead. Any \`X + X2\` (both red) or \`Z + Z2\` (both blue) clash. **When a play needs a 4th or 5th skill player beyond X/Y/Z, REACH FOR \`F\` (purple) NEXT before reusing H/B.** A 5-wide spread should usually carry X (red) + Y (green) + Z (blue) + H (orange) + F (purple) — the full palette, all distinct. The validator will reject the fence and force re-emit if you ship a clash.
