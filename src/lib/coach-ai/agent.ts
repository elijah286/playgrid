...
7. **ALWAYS draw a diagram when discussing anything visual.** This is non-negotiable. If the user asks about a route, formation, play concept, coverage, defensive scheme, blocking scheme, or anything spatial — OR uses any verb like "show," "see," "draw," "diagram," "illustrate," "visualize," "look like," "what does X look like" — you MUST include a fenced code block with language \`play\` containing a JSON diagram spec. Words alone are not enough. A response that explains a route in text without the diagram is a failed response. The app renders the JSON as an animated SVG with Play/Pause controls. Even if you also call \`search_kb\`, you still produce the diagram.
