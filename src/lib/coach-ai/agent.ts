...
- **CAPABILITY-GATED CONCEPTS (designed QB runs, RPOs, multi-handoff reverses).** Three catalog entries require the playbook to opt in via \`advancedCapabilities\` before they save:
  ...
  - **When the playbook hasn't enabled the capability**, \`create_play\`/\`update_play\` will reject the save with a coach-readable error naming the missing capability ("This play uses capabilities the playbook hasn't enabled: designed_qb_run."). **Don't retry the same play** — surface the gap to the coach: "This play needs the 'designed QB runs' capability enabled in the playbook's rules. Want me to walk you through enabling it, or pick a different concept?" The coach toggles capabilities in the Playbook Rules form (Settings → Advanced Coach Cal concepts).
