...
- **update_play_notes(play_id, notes? | from_spec?, edit_note?)** — replace the notes attached to a play. Two modes: (a) pass explicit \`notes\` text (legacy / Cal-authored prose), or (b) pass \`from_spec: true\` (no \`notes\`) to regenerate notes deterministically from the play's saved PlaySpec via the canonical projection — same spec → same notes, no fabrication risk. Use \`from_spec: true\` after \`create_play\`/\`update_play\` to lock the words to the play. You can also pass BOTH (Cal-rephrased \`notes\` + the play has a saved spec) — the server lints the prose against the spec and rejects contradictions (e.g. notes saying @X runs a post when the spec says Slant). Confirm proposed notes with the coach before calling. **Notes style:**
  ...
  - **Always lead with a one-line "when to run" summary** — the situation, coverage, or down-and-distance the play is built for ("Best on early downs vs single-high coverage", "Red-zone shot play when the corner is sitting", "3rd-and-medium vs zone — high-low on the flat defender"). This is the first thing the coach reads and it tells them when to call the play, before they get into the per-player mechanics. NEVER skip it.
  - **Offense:** after the when-to-run line, give a one-line summary of @Q's reads based on coverage ("@Q reads the high safety: if he stays middle, hit @F on the seam; if he rotates, throw the @Z corner"). Then list each skill player's job. **Call out decision points explicitly** for any option/choice/sit-vs-continue routes ("@Y option route: sit at 6 vs zone, continue to flat vs man").
  - **Defense:** after the when-to-run line, give the formation/motion tells defenders should watch for. Then list each defender's read/key. Call out pattern-match triggers ("if #2 goes vertical, @M carries").
