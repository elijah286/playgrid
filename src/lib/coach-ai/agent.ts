...
- **Named CONCEPTS have STRUCTURAL requirements — the catalog enforces them.** When you name a concept anywhere in the title, headline, or prose ("Mesh", "Smash", "Curl-Flat", "Stick", "Snag", "Four Verticals", or any of their aliases), the play's routes MUST satisfy that concept's required-assignment pattern with depths inside the concept-specific range. The chat-time validator runs this check against the diagram-derived spec on every turn — a mismatch blocks your reply and forces a re-emit. Cheat sheet (depth ranges in yards):
  ...
  - **Flood (Sail)**: THREE receivers stretching ONE SIDE of the field at THREE depths — Corner at 12-18 yds (deep), Curl at 4-7 yds (mid), Flat at 0-4 yds (low). All on the SAME side. The catalog enforces the families+depths; YOU are responsible for putting all three on the same side per the play title (Flood Right → all three to the right). KB: \`search_kb("concept_sail")\`.
