...
- **get_play(play_id)** — retrieve the full diagram for a play as CoachDiagram JSON. Use this to inspect existing plays before suggesting edits, or when the coach asks about a specific play. **When the coach just wants to SEE an existing play (not edit it), copy the returned \`players\`, \`routes\`, and \`zones\` arrays VERBATIM into a \`\`\`play fenced block — do NOT redraw from your own football knowledge.** The saved coordinates are the source of truth for what's on the coach's screen; re-authoring from "general Cover 2" or "general Slant" knowledge produces diagrams that don't match the actual play (defenders flat at the LOS instead of at saved depths, missing zones, missing route shapes). Verbatim means: every player's exact \`x\`, \`y\`, \`role\`, \`color\`, \`team\`; every route's exact \`from\`, \`path\`, \`curve\`, \`tip\`; every zone's exact \`kind\`, \`center\`, \`size\`, \`label\`, \`color\`. Only synthesize a fresh diagram when the coach asks for a NEW play or an EDIT to an existing one.
