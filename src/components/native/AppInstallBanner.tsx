...
import { getIosInstallCtaConfigAction } from "@/app/actions/admin-ios-install-cta";

export type InstallPlatform = "ios" | "android";

/** iOS CTA gate, read client-side from the Site Admin settings. Mirror of
 *  `IosInstallCtaConfig` (kept inline so this client module never imports the
 ...
 * always on. iOS is gated by the Site Admin toggle + a saved App Store ID
 * (see ios-install-cta-config.ts), read CLIENT-SIDE here via a server action —
 * deliberately NOT in the root layout, so nothing this banner does can perturb
 * static generation of other routes.
 */
export function AppInstallBanner() {
  // undefined while we decide on mount (the checks require the browser, so they
  // can't run during SSR); null once we've decided to hide.
  ...
  useEffect(() => {
    ...
    async function decide(): Promise<InstallBannerDecision | null> {
      ...
      const maxTouchPoints =
        typeof navigator.maxTouchPoints === "number"
          ...
          : 0;
      const isDismissed = (platform: InstallPlatform) => {
        try {
      ...
      // Only iOS needs the admin gate. Fetch it at runtime — and only for a
      // non-dismissed iOS visitor who could actually see the banner — so the
      // common cases make no network call and nothing runs during SSR/SSG.
      let iosInstallCta: IosInstallCtaProps | undefined;
      const prePlatform = isNative
        ? null
        : detectMobilePlatform(userAgent, maxTouchPoints);
      if (prePlatform === "ios" && !isDismissed("ios")) {
        try {
          const res = await getIosInstallCtaConfigAction();
          if (res.ok) {
            iosInstallCta = { enabled: res.enabled, appStoreId: res.appStoreId };
          }
        } catch {
          /* leave undefined → banner stays hidden */
        }
      }

      return resolveInstallBanner({
        isNative,
        userAgent,
        maxTouchPoints,
        iosInstallCta,
    ...
    decide()
      .then((next) => {
        ...
        if (next) {
          track({
            event: "app_banner_view",
            target: ANALYTICS_TARGET[next.platform],
          });
        }
      })
      .catch(() => {
