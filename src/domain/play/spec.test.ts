...
describe("Bug regression: flag_5v5 Hitch/Flat Combo — outside receiver illegal backwards route", () => {
  /**
   * 2026-05-04: Coach Cal generated "Spread Doubles — Hitch/Flat Combo" for flag_5v5.
   * Description: "hitch at 5 yards + flat checkdown."
   * Bug: X (outside receiver) rendered with a backwards-curving route that creates
   * an illegal forward-pass catch window (receiver settles deeper in backfield).
   *
   * In flag football, a forward pass must be caught at or beyond the LOS.
   * A receiver running backward (toward the backfield) violates this rule.
   *
   * Expected fix: either
   * (a) Reject the play as illegal (validator gate)
   * (b) Render X's route going FORWARD toward the catch point, not backward
   * (c) Don't assign the Hitch to X if X's role/position makes it illegal
   */
  it("flag_5v5: Hitch template should not produce backwards waypoints for an outside receiver", () => {
    const spec: PlaySpec = {
      schemaVersion: PLAY_SPEC_SCHEMA_VERSION,
      variant: "flag_5v5",
      ...
      assignments: [
        ...
        {
          player: "Y",
          action: { kind: "route", family: "Flat", depthYds: 2 },
        },
        {
          player: "C",
          action: { kind: "route", family: "Go", depthYds: 12 },
        },
        {
          player: "Z",
          action: { kind: "route", family: "Slant", depthYds: 5 },
        },
        { player: "Q", action: { kind: "none" } },
      ],
    };

    const { diagram } = playSpecToCoachDiagram(spec);
    const xRoute = diagram.routes?.find((r) => r.from === "X");
    const xPlayer = diagram.players.find((p) => p.id === "X");

    console.log("=== DEBUG: flag_5v5 Hitch/Flat Combo ===");
    console.log("X Player position:", xPlayer);
    console.log("X Route path:", xRoute?.path);
    if (xRoute?.path) {
      console.log(
        "X Route y-values:",
        xRoute.path.map((p) => p.y)
      );
    }

    expect(xRoute, "X should have a route assigned").toBeDefined();

    if (xRoute?.path && xRoute.path.length > 0) {
      // In flag_5v5, positive y typically means DEEPER toward the QB (the backfield).
      // For a forward pass to be legal, a receiver's route must have at least one
      // point at or beyond the LOS relative to the starting point.
      // A Hitch is a backwards/settling route, but the INITIAL motion should still be forward.

      const startY = xRoute.path[0]?.y ?? 0;
      const pathYValues = xRoute.path.map((p) => p.y ?? 0);
      const maxY = Math.max(...pathYValues);

      // For a Hitch on an outside receiver in a forward pass:
      // The route must have at least ONE waypoint where y <= startY (at or before the LOS).
      // If ALL waypoints are deeper (y > startY), the route is illegal.
      const hasForwardPoint = pathYValues.some((y) => y <= startY);

      console.log("startY:", startY, "maxY:", maxY, "hasForwardPoint:", hasForwardPoint);

      expect(
        hasForwardPoint,
        "Hitch on X must have at least one waypoint at or forward of the LOS (y <= start y)"
      ).toBe(true);
    }
  });

  it("flag_5v5: Flat on the back (Y) should be nearly horizontal, not upfield", () => {
    const spec: PlaySpec = {
      schemaVersion: PLAY_SPEC_SCHEMA_VERSION,
      variant: "flag_5v5",
      title: "Spread Doubles — Hitch/Flat Combo",
    ...
    const { diagram } = playSpecToCoachDiagram(spec);
    const yRoute = diagram.routes?.find((r) => r.from === "Y");

    expect(yRoute, "Y (back) should have a route assigned").toBeDefined();

    if (yRoute?.path && yRoute.path.length > 0) {
      const startY = yRoute.path[0]?.y ?? 0;
      const endY = yRoute.path[yRoute.path.length - 1]?.y ?? 0;
      const depthChange = Math.abs(endY - startY);

      // A Flat is a near-horizontal route (0-2 yards depth).
      // Normalized: 0.04 ≈ 1 yard, so 0.08 ≈ 2 yards.
      // Flat should have minimal depth change.
      expect(
        depthChange,
        "Flat on Y should be nearly horizontal (depth change < 0.08, i.e., < 2 yards)"
      ).toBeLessThan(0.08);
    }
  });
});
