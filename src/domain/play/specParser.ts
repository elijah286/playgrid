...
export function coachDiagramToPlaySpec(
  ...
  const defenderAssignments = buildDefenderAssignments(diagram);
...
/**
 * Build DefenderAssignment[] from the diagram's defender routes. A defender
 * with a route in the diagram round-trips to a `custom_path` action so the
 * waypoints survive the PlayDocument ↔ PlaySpec conversion. Defenders without
 * routes are NOT emitted — they inherit their default catalog assignment from
 * `spec.defense` at render time.
 *
 * This is the bridge between the canvas (a coach drags a path on a defender →
 * a Route lands on `doc.layers.routes`) and the spec layer Cal reads + writes.
 * Without it, defender movement drawn by hand would silently disappear from
 * the spec on every save → load cycle.
 */
function buildDefenderAssignments(diagram: CoachDiagram): DefenderAssignment[] {
  const result: DefenderAssignment[] = [];
  ...
  for (const r of diagram.routes ?? []) {
    ...
    const description = describeDefenderPath(r.from, path);
    result.push({
      defender: r.from,
      action: {
        kind: "custom_path",
        description,
        waypoints: path.map(([x, y]): [number, number] => [x, y]),
        ...(r.curve ? { curve: true } : {}),
      },
      confidence: "low",
    });
...
/** Short prose hint for hand-drawn defender movement. The full coaching line
 *  is generated by `notes-from-spec`; this is just enough to keep the spec
 *  self-describing when read in isolation. */
function describeDefenderPath(defender: string, path: Array<[number, number]>): string {
  const last = path[path.length - 1];
  const dx = last[0];
  const dy = last[1];
  const lateral = Math.abs(dx) > 1.5 ? (dx > 0 ? "right" : "left") : null;
  const vertical = Math.abs(dy) > 1.5 ? (dy > 0 ? "downfield" : "toward the LOS") : null;
  const direction = [vertical, lateral].filter(Boolean).join(" and ") || "to the marked spot";
  return `${defender} moves ${direction} at the snap`;
}
