...
export const SCHEMES: SchemeDef[] = [
  ...
  {
    id: "f7-cover-1-blitz",
    name: "7v7 Man · Cover 1 (1 Rush)",
    family: "scheme",
    variants: ["flag_7v7"],
    description: "7v7 Cover 1 with one rusher — 5 in man on the perimeter, 1 free safety deep, 1 rusher staged behind the 7-yard rush line.",
    body: "7v7 Cover 1 shell with a rusher — 1 rusher (staged behind the 7-yard rush line) attacks the QB, 5 defenders are in man on the 5 outside skill players (X, Z, H, S, B), and a free safety bails over the top. The default 7v7 pressure call when you want pass-rush without giving up the deep ball.",
    complexity: "intermediate",
    front: "7v7 Man",
    coverage: "Cover 1 Blitz",
    whenToUse: "Standard 7v7 pressure look — send one rusher to disrupt rhythm while keeping single-high safety help over the top. Best when the offense's primary read is a slow-developing inside concept; the rush forces them off-schedule.",
    weaknesses: [
      "Hot routes vs blitz — the rusher gives up an easy quick completion if the man defender doesn't break on the route.",
      "Picks and rubs from bunch / mesh — natural man-coverage weakness.",
      "Verticals on a slot mismatch — the FS only helps one side; the opposite slot is 1-on-1.",
    ],
    manCoverage: true,
    defenders: [
      ...
      { id: "CB", x: -12, y: 5,  assignment: { kind: "man", target: "X" } },
      { id: "NB", x: -6,  y: 5,  assignment: { kind: "man", target: "H" } },
      { id: "LB", x:  4,  y: 6,  assignment: { kind: "man", target: "B" } },
      { id: "NB", x:  6,  y: 5,  assignment: { kind: "man", target: "S" } },
      { id: "CB", x:  12, y: 5,  assignment: { kind: "man", target: "Z" } },
      { id: "FS", x:  0,  y: 13, assignment: { kind: "zone", zoneId: "deep_middle" } },
    ],
