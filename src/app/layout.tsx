    ...
    <html lang="en" className={`h-full scroll-smooth antialiased ${inter.variable}`} suppressHydrationWarning>
      ...
      <body className="flex min-h-[100dvh] flex-col bg-surface text-foreground font-sans [overflow-x:clip]">
        {/* Native loading overlay. Hidden on web (display:none unless
            ...
            fade it out. */}
        {/* Cold-launch launch frame: a static inline SVG — the app ground,
            soft blue/green brand blooms, and the Inter-italic wordmark —
            painted before hydration and styled in globals.css (scoped to
            #native-loading-overlay). It is the resolved frame of the native
            LaunchScreen (scripts/build-app-assets.mjs), so the native splash ->
            overlay handoff on cold launch is seamless. Theme-adaptive: the
            ground and wordmark follow html.dark, matching the themed splash. */}
        <div
          id="native-loading-overlay"
          aria-hidden="true"
          dangerouslySetInnerHTML={{
            __html: `<svg viewBox="0 0 1000 1000" preserveAspectRatio="xMidYMid slice" aria-hidden="true"><defs><radialGradient id="nlo-b1" cx="50%" cy="50%" r="50%"><stop offset="0%" stop-color="#1769FF" stop-opacity="0.42"/><stop offset="100%" stop-color="#1769FF" stop-opacity="0"/></radialGradient><radialGradient id="nlo-b2" cx="50%" cy="50%" r="50%"><stop offset="0%" stop-color="#95CC1F" stop-opacity="0.38"/><stop offset="100%" stop-color="#95CC1F" stop-opacity="0"/></radialGradient></defs><rect class="nlo-bg" x="0" y="0" width="1000" height="1000"/><g class="blooms"><circle cx="320" cy="400" r="550" fill="url(#nlo-b1)"/><circle cx="700" cy="620" r="550" fill="url(#nlo-b2)"/></g><text class="nlo-word" x="500" y="500" text-anchor="middle" dominant-baseline="central" font-style="italic" font-weight="800" font-size="56"><tspan fill="#1769FF">x</tspan><tspan fill="#95CC1F">o</tspan><tspan class="nlo-grid">gridmaker</tspan></text></svg>`,
          }}
        />
        {/* Overlay watchdog. Dismissal normally comes from NativeAppShell
            ...
            hydration never runs, and the coach stares at the launch screen
