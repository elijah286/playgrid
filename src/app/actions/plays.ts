...
/**
 * Install a catalog concept as a new play in the given playbook.
 * Generates the canonical skeleton → renders → saves, identical to the
 * pipeline the library page uses for display.
 */
export async function installConceptToPlaybookAction({
  conceptName,
  variant,
  playbookId,
}: {
  ...
  // Lazy-import to avoid pulling heavy domain code into the server-action
  // bundle unless this action is actually called.
  const { generateConceptSkeleton } = await import("@/domain/play/conceptSkeleton");
  const { playSpecToCoachDiagram } = await import("@/domain/play/specRenderer");
  const { coachDiagramToPlayDocument } = await import("@/features/coach-ai/coachDiagramConverter");

  const skeleton = generateConceptSkeleton(conceptName, { variant, strength: "right" });
  if (!skeleton.ok) return { ok: false, error: skeleton.error };

  const { diagram } = playSpecToCoachDiagram(skeleton.spec);
  const doc = coachDiagramToPlayDocument(diagram);
  doc.metadata.coachName = conceptName;
