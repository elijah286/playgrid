...
export async function sendPlaybookInviteEmailAction(input: {
  ...
  const subject = `You're invited to ${input.teamName} on XO Gridmaker`;
  const text = [
    sender ? `${input.senderName} invited you to join ${input.teamName} on XO Gridmaker.` : `You've been invited to join ${input.teamName} on XO Gridmaker.`,
    "",
...
<html><body style="font-family:ui-sans-serif,system-ui,Segoe UI,Helvetica,Arial,sans-serif;background:#f8fafc;margin:0;padding:24px">
  <table role="presentation" cellpadding="0" cellspacing="0" style="max-width:560px;margin:0 auto;background:#ffffff;border-radius:12px;overflow:hidden;border:1px solid #e2e8f0">
    <tr><td style="padding:24px">
      <h1 style="margin:0 0 8px;font-size:20px;color:#0f172a">Join ${team} on XO Gridmaker</h1>
 ...
 *
 * For each email:
 * - If the address already has a XO Gridmaker account, upsert an active
...
async function sendSharedExistingUserEmail(input: {
  ...
  const subject = `${input.teamName} was shared with you on XO Gridmaker`;
  const text = [
    sender
      ? `${input.senderName} shared ${input.teamName} with you on XO Gridmaker ${roleLabel}.`
      : `${input.teamName} was shared with you on XO Gridmaker ${roleLabel}.`,
    "",
...
<html><body style="font-family:ui-sans-serif,system-ui,Segoe UI,Helvetica,Arial,sans-serif;background:#f8fafc;margin:0;padding:24px">
  <table role="presentation" cellpadding="0" cellspacing="0" style="max-width:560px;margin:0 auto;background:#ffffff;border-radius:12px;overflow:hidden;border:1px solid #e2e8f0">
    <tr><td style="padding:24px">
      ...
      <p style="margin:0 0 16px;color:#475569;font-size:14px;line-height:1.5">
        ${sender ? `${sender} added you` : `You were added`} to ${team} on XO Gridmaker ${roleLabel}. It&rsquo;s already on your dashboard — open it below.
      </p>
      <p style="margin:0 0 20px">
