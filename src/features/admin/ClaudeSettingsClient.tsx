...
import {
  clearClaudeApiKeyAction,
  getClaudeIntegrationStatusAction,
  saveClaudeApiKeyAction,
  testClaudeApiKeyAction,
  clearAnthropicAdminKeyAction,
  getAnthropicAdminKeyStatusAction,
  saveAnthropicAdminKeyAction,
...
export function ClaudeSettingsClient({
  ...
  function refreshAdminStatus() {
    startTransition(async () => {
      const res = await getAnthropicAdminKeyStatusAction();
      if (!res.ok) {
  ...
  function refreshStatus() {
    startTransition(async () => {
      const res = await getClaudeIntegrationStatusAction();
      if (!res.ok) {
        toast(res.error, "error");
        return;
      }
  ...
  return (
    <Card className="p-5">
      ...
      >
        <div>
          <h3 className="text-base font-semibold text-foreground">Anthropic Claude</h3>
          <p className="mt-0.5 text-xs text-muted">
            Powers Coach AI chat when the active provider is Claude. {statusLabel}
          </p>
        </div>
          ...
          <div className="mt-4 flex flex-wrap items-center justify-between gap-2">
            <div className="flex flex-wrap gap-2">
              <Button
                ...
                onClick={() => {
                  startTransition(async () => {
                    const res = await saveClaudeApiKeyAction(draftKey);
                    if (!res.ok) {
                      toast(res.error, "error");
                      return;
                    }
              ...
              <Button
                ...
                onClick={() => {
                  startTransition(async () => {
                    const res = await testClaudeApiKeyAction(draftKey || undefined);
            ...
            {configured && (
              <Button
                ...
                disabled={pending}
                onClick={() => {
                  if (!globalThis.confirm("Remove the saved Claude API key?")) return;
                  startTransition(async () => {
                    const res = await clearClaudeApiKeyAction();
                    if (!res.ok) {
                      toast(res.error, "error");
                      return;
                    }
          ...
          <div className="mt-6 border-t border-black/5 pt-4">
            ...
            <div className="mt-3">
              <label className="block">
                ...
                <Input
                  ...
                  placeholder={adminConfigured ? "••••••••••••" : "sk-ant-admin-…"}
                />
              </label>
            </div>
            <div className="mt-3 flex flex-wrap items-center justify-between gap-2">
              <Button
                ...
                onClick={() => {
                  startTransition(async () => {
                    const res = await saveAnthropicAdminKeyAction(adminDraftKey);
                    if (!res.ok) {
                      toast(res.error, "error");
                      return;
                    }
              ...
              {adminConfigured && (
                <Button
                  ...
                  disabled={pending}
                  onClick={() => {
                    if (!globalThis.confirm("Remove the saved Anthropic admin key?")) return;
                    startTransition(async () => {
                      const res = await clearAnthropicAdminKeyAction();
                      if (!res.ok) {
                        toast(res.error, "error");
                        return;
                      }
